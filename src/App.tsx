...
import { createBackgroundRenderer, isBackgroundAligned } from './renderer/backgroundRenderer'
  ...
  useLayoutEffect(() => {
    ...
    const draw = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        const rasterBackground = backgroundRef.current?.update(mapArea, view, canvas.clientWidth, canvas.clientHeight, [...currentSelectedIds])
        // A stale 2D bitmap is visibly blurry while panning. Keep the vector
        // renderer active until the worker has produced the exact camera view;
        // iso keeps the old raster fallback because its depth layers are much
        // more expensive to redraw during a gesture.
        const background = mode === '2d' && rasterBackground && !isBackgroundAligned(rasterBackground, view) ? undefined : rasterBackground
