...
function App() {
  ...
  useEffect(() => {
    ...
    if (query.length < 2 || originCoordinates) { setOriginSuggestions([]); return }
    ...
    const draw = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        const background = mode === 'iso'
          ...
          : undefined
        renderMap(canvas, mapArea, view, activeCity.landmarks, { visibleSportIds: currentVisibleIds, selectedSportIds: currentSelectedIds, selectionProgress, trendingEnabled, trendingSignals }, { background, mapLabels: activeCity.mapLabels, route })
      })
    }
  ...
  }, [mapArea, mode, filter, priceFilter, accessibilityFilter, familyFilter, searchQuery, selected, selectionProgress, trendingEnabled, trendingSignals, route])

  useEffect(() => {
    const canvas = canvasRef.current
    if (!canvas || !mapArea || !selected?.center) return
    const rect = canvas.getBoundingClientRect()
    const current = cameraRef.current
    const nextZoom = Math.max(current.zoom, mode === 'iso' ? 2.55 : 1.75)
    const projector = createProjector(mapArea, rect.width, rect.height, { mode, zoom: nextZoom, pan: current.pan })
    const point = projector.point(toLocalMetres(selected.center, mapArea.center))
    cameraRef.current = { zoom: nextZoom, pan: { x: current.pan.x + rect.width / 2 - point.x, y: current.pan.y + rect.height / 2 - point.y } }
    scheduleDrawRef.current?.()
    scheduleViewportLoad({ width: rect.width, height: rect.height, mode, zoom: nextZoom, pan: cameraRef.current.pan })
  }, [mapArea, mode, selected?.id])

  useEffect(() => {
    if (!selected) { setSelectionProgress(0); return }
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) { setSelectionProgress(1); return }
    let frame = 0
    ...
    <section className="map-stage">
      ...
      {trendingEnabled && topTrending.length > 0 && <div className="trending-rail" aria-label={text(locale, 'trendingNow')}>
        ...
        <div className="trending-items">{topTrending.map(({ feature, signal }, index) => {
          ...
          return <button key={feature.id} type="button" className={`trending-item trend-${signal.stage} ${selected?.id === feature.id ? 'selected' : ''}`} onClick={() => setSelected(feature)} aria-pressed={selected?.id === feature.id}>
