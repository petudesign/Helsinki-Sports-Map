...
function App() {
  ...
  const visibleSports = mapArea?.sports.filter((feature) => {
    ...
    return [feature.name, feature.facilityType, feature.sport, ...(feature.sports ?? [])].filter(Boolean).some((value) => value?.toLocaleLowerCase('fi-FI').includes(normalizedSearch))
  }) ?? []
  ...
  const selectedVenue = selected && mapArea ? venueForFeature(mapArea.venues, selected) : undefined
  ...
  const zoomAt = (event: WheelEvent<HTMLCanvasElement>, direction: number) => {
    event.preventDefault()
    const rect = event.currentTarget.getBoundingClientRect()
    zoomTo({ x: event.clientX - rect.left, y: event.clientY - rect.top }, direction)
  }
